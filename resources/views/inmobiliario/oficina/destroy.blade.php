            ...
            <div class="modal-content">
                @if(count($oficina->articulo) > 0)
                    ...
                    <div class="modal-body">
                        <p>{{'Tiene '.count($oficina->articulo).' artículos dependientes, edítelos para que no dependan de esta oficina y evitar errores'}}</p>
                    </div>
                ...
                <div class="modal-footer">
                    <form action="{{route('inmobiliario.'.$nombre_concepto.'.destroy',$oficina->id)}}" method="post" class="d-inline-block" id="eliminar">
                        {{method_field('DELETE')}}
                        @csrf
