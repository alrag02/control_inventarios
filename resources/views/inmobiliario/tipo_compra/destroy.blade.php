        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{route('inmobiliario.'.$nombre_concepto.'.destroy',$tipo_compra->id)}}" method="post" class="d-inline-block" id="eliminar">
                        {{method_field('DELETE')}}
                        @csrf
                ...
                <?php /*
                @if( count($tipo_compra->articulo_has_tipo_compra) <= 0 )
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">¿Está seguro?</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                ...
                <div class="modal-body">
                    <p>{{'Tiene '.count($tipo_compra->articulo_has_tipo_compra).' articulo_has_tipo_compras dependientes, editelos para que no dependan de esta tipo_compra y evitar errores'}}</p>
                </div>
                <div class="modal-footer">
                    <a href="{{route('inmobiliario.articulo_has_tipo_compra.index')}}">
