        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{route('inmobiliario.'.$nombre_concepto.'.destroy',$tipo_equipo->id)}}" method="post" class="d-inline-block" id="eliminar">
                        {{method_field('DELETE')}}
                        @csrf
                ...
                <?php /*
                @if( count($tipo_equipo->articulo_has_tipo_equipo) <= 0 )
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">¿Está seguro?</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                ...
                <div class="modal-body">
                    <p>{{'Tiene '.count($tipo_equipo->articulo_has_tipo_equipo).' articulo_has_tipo_equipos dependientes, editelos para que no dependan de esta tipo_equipo y evitar errores'}}</p>
                </div>
                <div class="modal-footer">
                    <a href="{{route('inmobiliario.articulo_has_tipo_equipo.index')}}">
